...
export const createCase = async (req, res) => {
  try {
    const { caseName, description_text, notes, reportTitle, startDate, endDate } =
      req.body;
    ...
    if (typeof caseName !== "string" || caseName.trim() === "") {
      return res
        .status(400)
        .json({ message: "case name must be a non-empty string" });
    }

    if (
      typeof description_text !== "string" ||
      description_text.trim() === ""
    ) {
      return res
        .status(400)
        .json({ message: "description text must be a non-empty string" });
    }

    if (typeof reportTitle !== "string" || reportTitle.trim() === "") {
      return res
        .status(400)
        .json({ message: "report title must be a non-empty string" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "start date and end date must be valid date strings" });
    }
    ...
    if (period.length === 0) {
      return res
        .status(400)
        .json({ message: "No OPEN training period found for the given dates" });
    }
    ...
    if (startDate !== undefined && endDate !== undefined) {
      ...
      if (period.length === 0) {
        return res.status(400).json({
          message: "No OPEN training period found for the given dates",
        });
      }
